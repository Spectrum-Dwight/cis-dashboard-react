...
function BarWidget({ loading, currentIndex, removeWidget, accountDropDownOptions, barChartData }: BarWidgetProps) {
    ...
    if (barChartData) {
        ...
        if (selectedAccountId === 0) {
            ...
            filteredBarData = barChartData.filter(account => account.AccountID === selectedAccountId);
        }
...
function createBarChartData(totals: any) {
    ...
    const labels = Array.from({ length: 5 }, (_, index) => `${currentYear - index}`).reverse();
